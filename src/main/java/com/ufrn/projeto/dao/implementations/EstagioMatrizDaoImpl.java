...
//    }
  ...
  public void saveLog(int id, String e){
      try {
            ...
            Matriz matriz = mDao.findById(id);
            
            EnumEstagio enumestagio = EnumEstagio.getETipoFatura(e);
            
            LogEstagio log = new LogEstagio(matriz, enumestagio);
            estagioDao.save(log);    
            t.commit();
      } catch (HibernateException erro) {
            System.out.println("Erro listando: " + erro);
      } finally{
            session.close();
      }
  }
